// @/components/OAuthButton.tsx
'use client'

import { motion } from 'framer-motion'

interface OAuthButtonProps {
  provider: 'google'
  onClick: () => void
  isLoading?: boolean
}

export function OAuthButton({ provider, onClick, isLoading = false }: OAuthButtonProps) {
  const providerConfig = {
    google: {
      name: 'Google',
      icon: (
        <svg className="w-5 h-5" viewBox="0 0 24 24">
          <path
            fill="currentColor"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="currentColor"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84c1.81 1.58 4.52 2.52 7.82 2.52z"
          />
          <path
            fill="currentColor"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="currentColor"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
      ),
      bgColor: 'bg-white hover:bg-gray-50 dark:bg-slate-800 dark:hover:bg-slate-700',
      textColor: 'text-gray-900 dark:text-slate-100',
      borderColor: 'border-gray-300 dark:border-slate-600'
    }
  }

  const config = providerConfig[provider]

  return (
    <motion.button
      whileHover={{ scale: isLoading ? 1 : 1.02 }}
      whileTap={{ scale: isLoading ? 1 : 0.98 }}
      onClick={onClick}
      disabled={isLoading}
      className={`
        w-full flex items-center justify-center gap-3 px-4 py-3 rounded-2xl font-medium 
        transition-all duration-200 border focus:outline-none focus:ring-2 focus:ring-offset-2 
        focus:ring-slate-500 disabled:opacity-50 disabled:cursor-not-allowed
        ${config.bgColor} ${config.textColor} ${config.borderColor}
      `}
    >
      {isLoading ? (
        <div className="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin" />
      ) : (
        <div className="text-gray-700 dark:text-slate-100">
          {config.icon}
        </div>
      )}
      <span>
        {isLoading ? 'Connecting...' : `Continue with ${config.name}`}
      </span>
    </motion.button>
  )
}